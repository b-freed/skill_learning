...
from skill_model import LowLevelPolicy, AutoregressiveLowLevelPolicy,Encoder,Prior,AbstractDynamics,GenerativeModel
import os
...
class SkillPolicyModel(nn.Module):
	...
	def __init__(self,state_dim,a_dim,z_dim,h_dim,beta,a_dist,fixed_sig=None):
		...
		if a_dist == 'autoregressive':
			self.ll_policy = AutoregressiveLowLevelPolicy(state_dim,a_dim,z_dim,h_dim,max_sig=None,fixed_sig=fixed_sig)
		else:
...
class DynamicsModel(nn.Module):
	def __init__(self,state_dim,a_dim,z_dim,h_dim,beta,per_element_sigma):
		super().__init__()
		self.dynamics_model = AbstractDynamics(state_dim,z_dim,h_dim,init_state_dependent=True,per_element_sigma=per_element_sigma)
		self.encoder = Encoder(state_dim,a_dim,z_dim,h_dim)
		self.prior = Prior(state_dim,z_dim,h_dim)
		self.beta = beta
		self.gen_model = GenerativeModel(self.dynamics_model,self.prior) # this is just a container so we can easily access params we update in M step
	...
	def get_losses(self,states,actions):
		...
		s_T_mean, s_T_sig = self.dynamics_model(s_0,z_sampled)
		# Get prior.
...
class TwoStageSkillModel(nn.Module):
	...
	def __init__(self,state_dim,a_dim,z_dim,h_dim,a_dist,stage,path,skill_beta=1.0,model_beta=1.0,fixed_sig=None,per_element_sigma=True,lr=5e-5,wd=0.0):
		...
		self.dynamics_model = DynamicsModel(state_dim,a_dim,z_dim,h_dim,model_beta,per_element_sigma)
	...
	def train(self,data):
		...
		if self.stage == 'train_skills':
			...
			loss.backward()
			# print(self.skill_model.encoder.emb_layer[0].weight.grad)
			# print(loss.grad)
			self.optimizer.step()
			# print('loss: ', loss)
			return loss,a_loss,kl_loss
		...
		elif self.stage == 'train_dynamics':
			########### E STEP ###########
	...
	def get_E_loss(self,states,actions):
		...
		assert self.stage == 'train_dynamics'
		...
		if self.skill_model.a_dist != 'autoregressive': 
			a_means, a_sigs = self.skill_model.ll_policy(states,z_sampled)
		else:
			a_means, a_sigs = self.skill_model.ll_policy(states,actions,z_sampled)
		a_dist = Normal.Normal(a_means, a_sigs)
		# a_means,a_sigs = self.skill_model.ll_policy(states,z_sampled)
	...
	def get_M_loss(self,states,actions):
		...
		z_prior_means,z_prior_sigs = self.dynamics_model.prior(s_0) 
		sT_mean, sT_sig = self.dynamics_model.dynamics_model(s_0,z_sampled)
		...
		return sT_loss + self.model_beta*prior_loss
