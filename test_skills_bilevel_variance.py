...
if __name__ == '__main__':
    ...
    filename = 'Maze2d_bilevel_noisy_H20_l2reg_0.0_log_best.pth'
    PATH = 'checkpoints/'+filename
    ...
    for j in range(n_skills):
        ...
        initial_state = env.reset()
        state = torch.reshape(torch.tensor(initial_state,dtype=torch.float32).cuda(), (1,1,state_dim))
        ...
        #   sT_mean,sT_sig = skill_model.decoder.abstract_dynamics(states[:,0:1,:],z)
            ...
            finals.append(states_actual[-1])
        sT_mean = np.mean(finals, axis=0)
        sT_sig = np.std(finals, axis=0) * 3
        print(sT_mean)
        print(sT_sig)
        actual_states.append(states_actual)
        action_dist.append(actions)
        ...
        state = torch.reshape(torch.tensor(initial_state,dtype=torch.float32).cuda(), (1,1,state_dim))
#        sT_mean, sT_sig = skill_model_sdp.decoder.abstract.forward(state, z)
#        sT_mean = sT_mean.flatten()
#        sT_sig = sT_sig.flatten()
#        print(sT_mean)
#        print(sT_sig)
        plt.scatter(sT_mean[0].item(), sT_mean[1].item(),color=colors[2])
        plt.gca().add_patch(Ellipse((sT_mean[0].item(), sT_mean[1].item()), width=sT_sig[0].item(), height=sT_sig[1].item(), edgecolor="blue", facecolor='none'))
        
    
    plt.title('Skill Variance (Skill-Dependent Prior)')
    plt.axis('square')
    # plt.savefig('Skill_Prediction_H'+str(H)+'_'+str(i)+'.png')
    plt.savefig('Skill_Prediction_H'+str(H)+'_bilevel_variance.png')
