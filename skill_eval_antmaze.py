...
def run_skill(skill_model,s0,skill,env,render,pred_state,timesteps):
	...
	frames = []
	min_dist = 10**10
	j_min_dist = timesteps
	for j in range(timesteps): #H-1 if H!=1
	# for j in range(100):
		if render:
		...
		states.append(state)
		if np.sum((state[:2] - pred_state[:2])**2) < min_dist:
			min_dist = np.sum((state[:2] - pred_state[:2])**2)
			j_min_dist = j 
	  
	return np.stack(states)[:j_min_dist,:],np.stack(actions)[:j_min_dist,:],frames[:j_min_dist], min_dist
...
if __name__ == '__main__':
	...
	H = 20
	N,state_dim = data['observations'].shape
	...
	if not state_dependent_prior:
		...
		skill_model_sdp = SkillModelStateDependentPrior(state_dim, a_dim, z_dim, h_dim, a_dist=a_dist).cuda()
	...
	for i in range(episodes):
		...
		initial_state = observations[j,:]# choose random state from dataset
	...
	# 	sT_mean,sT_sig = skill_model.decoder.abstract_dynamics(states[:,0:1,:],z)
		...
		plt.scatter(states_actual[0,0],states_actual[0,1],c='g')
		plt.errorbar(sT_mean[0,0,0].detach().cpu().numpy(),sT_mean[0,0,1].detach().cpu().numpy(),xerr=sT_sig[0,0,0].detach().cpu().numpy(),yerr=sT_sig[0,0,1].detach().cpu().numpy(),c='b')
	...
	print('np.mean(min_dists): ',np.mean(min_dists))
