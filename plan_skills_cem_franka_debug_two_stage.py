...
def run_skill(skill_model,s0,skill,env,H,render,use_epsilon=False):
	...
	for j in range(H): #H-1 if H!=1
		...
		action = skill_model.skill_model.ll_policy.numpy_policy(state,z)
...
if __name__ == '__main__':
	...
	h_dim = 256
	z_dim = 256
	batch_size = 100 # irrelevant parameter
	# episodes = 4
	wd = .01 # irrelevant parameter
	lr = 1e-5   # irrelevant parameter
	...
	render = True
	stage = 'train_dynamics' # irrelevant parameter
	...
	# filename = 'EM_model_05_24_22_kitchen-partial-v0state_dec_mlp_init_state_dep_True_H_40_l2reg_0.0_a_1.0_b_1.0_per_el_sig_False_a_dist_autoregressive_log_best.pth'
	filename = 'two_stage_05_20_22_kitchen-partial-v0_H_40_l2reg_0.0_skill_b_0.1_model_b_1.0_per_el_sig_False_a_dist_autoregressive'
	...
	skill_model = TwoStageSkillModel(state_dim,a_dim,z_dim,h_dim,a_dist,stage,path,skill_beta=1.0,model_beta=1.0,fixed_sig=fixed_sig,per_element_sigma=per_element_sigma,lr=lr,wd=wd).cuda()


	# checkpoint = torch.load(PATH)
	# skill_model.load_state_dict(checkpoint['model_state_dict'])
	skill_model.load_from_checkpoint('log_best.pth')
	...
	for i in range(1):
		...
		for j in range(skill_seq_len):
			...
			mu_z, sigma_z = skill_model.dynamics_model.prior(torch.tensor(state,device=torch.device('cuda:0'),dtype=torch.float32).unsqueeze(0))
			print('mu_z: ', mu_z)
			...
			sT_mean,sT_sig = skill_model.dynamics_model.dynamics_model(torch.tensor(state,device=torch.device('cuda:0'),dtype=torch.float32).unsqueeze(0),z)
			print('sT_sig: ', sT_sig)
			...
			make_video(frames,'debug_franka_H_'+str(H)+'_'+str(i)+'_'+filename)
			plt.figure()
