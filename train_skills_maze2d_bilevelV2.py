...
def train(model,model_optimizer, device=torch.device("cpu")):
...
if not noisy:
    ...
    dataset_file = "data/maze2d-large-v1-noisy-1.hdf5"
    dataset = h5py.File(dataset_file, "r")  # dictionary, with 'observations', 'rewards', 'actions', 'infos/goal'
...
model = BilevelSkillModelV2(state_dim,a_dim,z_dim,h_dim).to(device)
...
if not noisy:
    name_base = "Maze2d_bilevel_no_noise"
else:
    name_base = "Maze2d_bilevel_noisy"

min_test_loss = 10**10
for i in range(n_epochs):
    loss, s_loss, a_loss, s_T_loss, kl_loss = train(model,model_optimizer, device)
    ...
    #    loss,      s_loss,      a_loss,      s_T_loss,      kl_loss
    test_loss, test_s_loss, test_a_loss, test_s_T_loss, test_kl_loss = test(model, device)
    ...
    if i % 10 == 0:
        ...
        filename = name_base + '_H'+str(H)+'_l2reg_'+str(wd)+'_log.pth'
    ...
    if test_loss < min_test_loss:
        ...
        filename = name_base + '_H'+str(H)+'_l2reg_'+str(wd)+'_log_best.pth'
