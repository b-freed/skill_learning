...
class HyperParams:
    def __init__(self):
        ...
        self.exp_name = exp_name = "T_10_20_slp100"
        ...
        run_num = 0
        self.exp_name = f'{exp_name}_r{run_num}'
        while os.path.exists(os.path.join(self.log_dir, self.exp_name)):
            run_num += 1
            self.exp_name = f'{exp_name}_{run_num}'
...
if hp.term_state_dependent_prior:
	...
	model = SkillModelStateDependentPriorAutoTermination(state_dim, a_dim, hp.z_dim, hp.h_dim, a_dist=hp.a_dist,state_dec_stop_grad=hp.state_dec_stop_grad,beta=hp.beta,alpha=hp.alpha,gamma=hp.gamma,max_sig=hp.max_sig,fixed_sig=hp.fixed_sig,ent_pen=hp.ent_pen,encoder_type=hp.encoder_type,state_decoder_type=hp.state_decoder_type).to(hp.device)
else:
